...
- For any supported model, a consumer can obtain that model's native reasoning term, the values it accepts (a discrete set, or a valid token-budget range) and its default — without any provider-specific knowledge of its own.
