...
**Baked-in rate tables** (nano-USD/token; gathered from official provider pricing 2026-06-17 per research §6.5 — re-verify before release). Each row is one `RateTier`; tiered models have two. Anthropic cache-write rates are derived from Anthropic's conventional 0.1×/1.25×/2× multipliers (base input/output published, high confidence); all other providers have no cache-write bucket (0). **gpt-5.5-pro has no cached-input discount**, so its `CacheReadInput` rate equals `InputUncached` (cached reads bill at full input rate). **`gpt-5.6-sol/terra/luna` ship a single base tier** — no >272K high-tier rate is published for the 5.6 line, so unlike the tiered gpt-5.5/gpt-5.4 they carry one `RateTier`; the missing high tier is a re-verify-before-release item. **`claude-sonnet-5` is priced at its standard $3/$15 sticker** (3000/15000), not the introductory promo — the table is the durable contract, so a dated promo is deliberately not baked in.

| Model | MinInputTokens | InputUncached | CacheReadInput | CacheWrite5m | CacheWrite1h | Output |
|---|---|---|---|---|---|---|
| claude-fable-5 | 0 | 10000 | 1000 | 12500 | 20000 | 50000 |
| claude-opus-4-8 | 0 | 5000 | 500 | 6250 | 10000 | 25000 |
| claude-sonnet-5 | 0 | 3000 | 300 | 3750 | 6000 | 15000 |
...
| gpt-5.6-sol | 0 | 5000 | 500 | 0 | 0 | 30000 |
| gpt-5.6-terra | 0 | 2500 | 250 | 0 | 0 | 15000 |
| gpt-5.6-luna | 0 | 1000 | 100 | 0 | 0 | 6000 |
...
|---|---|---|---|---|---|---|
| claude-fable-5 | effort | Enum | low, medium, high, xhigh, max | — | `Level("medium")` | no |
| claude-opus-4-8 | effort | Enum | low, medium, high, xhigh, max | — | `Level("high")` | yes |
| claude-sonnet-5 | effort | Enum | low, medium, high, xhigh, max | — | `Level("medium")` | yes |
...
| gpt-5.6-sol | effort | Enum | none, low, medium, high, xhigh *(est.)* | — | `Level("medium")` *(est.)* | yes |
| gpt-5.6-terra | effort | Enum | none, low, medium, high, xhigh *(est.)* | — | `Level("medium")` *(est.)* | yes |
| gpt-5.6-luna | effort | Enum | none, low, medium, high, xhigh *(est.)* | — | `Level("medium")` *(est.)* | yes |
...
- **Anthropic effort default is `high`** for the 4.x models (display/fallback); the thinking-block axis (off until a level engages `type:"adaptive"`) is handled by the D9 lowering, not the spec value. **`claude-fable-5` and `claude-sonnet-5` deliberately carry `Default = Level("medium")`** — this is AgentKit's chosen warn-fallback value, not the provider's documented default (Anthropic documents `high`); `medium` is a member of each model's own `Levels`, so R-EPIG-0UQT holds. `claude-fable-5` is `CanDisable=false` (thinking is always on; a `DisableReasoning()` warns and falls back to the default per D6), `claude-sonnet-5` is `CanDisable=true`.
- *(est.)* cells (gpt-5.5-pro levels/default, gpt-5.4-mini/nano defaults, gpt-5.6-sol/terra/luna levels/default, gemini-3.1-flash-lite default) are research estimates — the plan/release process verifies them against a live provider response before release, exactly as for the lower-confidence pricing rows.
...
- R-CDK0-SUGR — the `openai` registry exports `ModelGPT56Sol`/`ModelGPT56Terra`/`ModelGPT56Luna` and prices each at this decision's single-tier rates (sol 5000/500/30000, terra 2500/250/15000, luna 1000/100/6000; each with exactly **one** `RateTier`, `MinInputTokens==0`, and both cache-write buckets 0) — a second tier or a mismatched rate fails.
- R-CERX-6M7G — the `openai` `ReasoningSpec` for `gpt-5.6-sol`/`gpt-5.6-terra`/`gpt-5.6-luna` is `Kind=Enum`, `Levels=[none,low,medium,high,xhigh]`, `Default=Level("medium")`, `CanDisable=true` for all three — a default outside that level set or a wrong `CanDisable` fails.
- R-CH7P-Y5OU — the `anthropic` registry exports `ModelFable5`/`ModelSonnet5` and prices each at this decision's rates including the cache-write buckets (fable-5 10000/1000/12500/20000/50000; sonnet-5 3000/300/3750/6000/15000, the standard sticker not the intro) — a zeroed cache-write bucket or an intro-rate output fails.
- R-CIFM-BXFJ — the `anthropic` `ReasoningSpec` for `claude-fable-5` and `claude-sonnet-5` is `Kind=Enum`, `Levels=[low,medium,high,xhigh,max]`, `Default=Level("medium")`, and the two differ on `CanDisable` (`claude-fable-5`=false, `claude-sonnet-5`=true) — swapping either `CanDisable` value fails.
