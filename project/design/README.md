...
- **Dependencies.** External modules require **explicit per-case human approval**; the standard library is the default. Approved today: `github.com/invopop/jsonschema` (struct→JSON-Schema derivation in `NewTool`, D4) and its transitive closure. Provider SDKs, the MCP `go-sdk`, and backoff libraries are **not** approved — every provider adapter and the MCP client are raw `net/http` over `internal/httpx`. Adding a module is an operator decision, never a build-loop one.
