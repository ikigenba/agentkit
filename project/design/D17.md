...
- **Schema-lossiness warnings are provider-driven, never name-dispatched.** A discovered MCP tool's JSON-Schema is **faithfully translated** into the selected provider's dialect; only a construct with no faithful representation surfaces a `WarnToolSchemaLossy` warning at this boundary. The core does **not** branch on `Provider.Name()`: it type-asserts the optional `ToolSchemaTranslator` capability interface (D22) and asks the provider what it cannot faithfully represent, attributing the warning per `<server>.<originalName>`. The translation and the residue knowledge live in the provider adapter (the Gemini dialect in `google`), not in the orchestration core; a provider that does not implement `ToolSchemaTranslator` yields no schema warnings.
