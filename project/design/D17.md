...
- **A server's schema is gated, not translated-with-warnings.** A discovered MCP tool's `inputSchema` is third-party bytes and is the one schema source AgentKit does not generate, so it is the only source that can leave the canonical subset. It is validated against that subset at the `Send` boundary exactly like any other tool (D34): a schema outside it fails the whole `Send` with `ErrInvalidConfig` attributing `<server>.<originalName>`, rather than being dropped, degraded, or warned about. Every MCP server AgentKit connects to is first-party, so a non-conforming schema is a bug to fix at the server, not a condition to tolerate. Within the subset, rendering to the selected provider's dialect is the adapter's job (D22) and cannot fail.
