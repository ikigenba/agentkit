...
internal/openaicompat — shared OpenAI Chat-Completions adapter; the public `zai` and `openrouter` packages (and any future compat-backed provider) construct it with their own baked-in base URLs and per-provider configuration (cost extraction, reasoning encoding). Not consumer-importable.
