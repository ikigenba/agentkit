...
2. Build a `Request` from the `Conversation` (System, Messages=History, Tools, Gen, Model), with **tools serialized in stable deterministic order** — the name-sorted base set, followed by any deferred tools already loaded, in load order (Decision 23) — for cache stability.
3. Loop:
...
- **Unknown tool name → `ToolResultBlock{IsError: true}` fed back**, not fatal — consistent with Decision 4. (A *deferred-but-unloaded* name is a distinct case with its own corrective path, owned by Decision 23.)
...
- R-VXPR-861V — the base toolset is serialized into the `Request` in name-sorted, deterministic-JSON order, identical across successive turns of a conversation (a deferred load may only extend it at the tail; the append-order guarantee is Decision 23's R-DBTB-4XLB).
