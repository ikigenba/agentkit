...
- R-LNO6-EM5R — a `turn_start` record emits `provider` and `auth` as two separate JSON string fields taken from the conversation provider's `Identity`: an OpenAI subscription turn writes `"provider":"openai","auth":"subscription"` and an OpenAI API-key turn writes `"provider":"openai","auth":"apikey"` — the same `provider` value for both — while a Z.ai turn writes `"provider":"z-ai"`. Asserted on the raw log bytes, so no record carries a dotted composite in `provider`.
