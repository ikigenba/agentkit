...
type LogRecord struct {
    ...
    Provider ProviderID `json:"provider,omitempty"` // turn_start: the provider package (D9)
    Auth     AuthMode   `json:"auth,omitempty"`     // turn_start: the credential mode (D9/D25)
    Model    string     `json:"model,omitempty"`    // turn_start
    Status   string     `json:"status,omitempty"`   // turn_end
}
```
...
- **`turn_start` carries provider and auth as two fields.** The record takes the conversation provider's `Identity` (D9) split into `provider` and `auth`, rather than one rendered label. A consumer filtering a log by provider would otherwise have to know that `openai` is sometimes written `openai.apikey`; two fields make "every OpenAI turn" and "every subscription-paid turn" independent filters. Both are defined string types, so each serializes as a plain JSON string and the schema stays flat.
...
- R-LNO6-EM5R — a `turn_start` record emits `provider` and `auth` as two separate JSON string fields taken from the conversation provider's `Identity`: an OpenAI subscription turn writes `"provider":"openai","auth":"subscription"` and an OpenAI API-key turn writes `"provider":"openai","auth":"apikey"` — the same `provider` value for both — while an xAI subscription turn writes `"provider":"x-ai","auth":"subscription"` and a Z.ai turn writes `"provider":"z-ai"`. Asserted on the raw log bytes, so no record carries a dotted composite in `provider`.
