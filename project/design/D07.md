...
3. **Boundary-validation errors** — `ErrInvalidConfig`, `ErrInvalidInput` (D1), `ErrMissingCredential` (D38), raised before any provider call — are **bare** sentinels.
...
type Error struct {
    ...
    Provider   ProviderID      // which provider package failed; "" for MCP failures
    Auth       AuthMode        // which credential mode it was using; "" for MCP failures
...
- **`Provider` and `Auth` are two typed fields, not one label.** They carry the failing handle's `Identity` (D9) split into its parts: which provider package, and which credential mode it was paying with. Both are defined string types, so the JSON shape of a serialized error is unchanged — a `provider` string and an `auth` string, not a struct. They remain diagnostic detail decoupled from the `Provider` handle *interface*; what changed is that the diagnostic no longer collapses two facts into a dotted string a reader has to split. D25 requires the modes be distinguishable, because their cost semantics differ; two fields distinguish them without making `provider` mean `openai` sometimes and `openai.apikey` other times.
...
**Verification.**
- R-LMGA-0UF2 — a provider failure's `*Error` carries `Provider` and `Auth` as two separate typed fields matching the failing handle's `Identity`: an `openai` subscription failure has `Provider == ProviderOpenAI` and `Auth == AuthSubscription` (never a dotted `Provider` value), an `openai` API-key failure has the same `Provider` with `Auth == AuthAPIKey`, an `xai` subscription failure has `Provider == ProviderXAI` and `Auth == AuthSubscription`, and a `z-ai` failure has `Provider == ProviderZAI`. An MCP failure leaves **both** empty while `MCPServer` is set. Serialized to JSON the two land as separate `provider` and `auth` strings.
