...
**Verification.**
- R-LMGA-0UF2 — a provider failure's `*Error` carries `Provider` and `Auth` as two separate typed fields matching the failing handle's `Identity`: an `openai` subscription failure has `Provider == ProviderOpenAI` and `Auth == AuthSubscription` (never a dotted `Provider` value), an `openai` API-key failure has the same `Provider` with `Auth == AuthAPIKey`, and a `z-ai` failure has `Provider == ProviderZAI`. An MCP failure leaves **both** empty while `MCPServer` is set. Serialized to JSON the two land as separate `provider` and `auth` strings.
