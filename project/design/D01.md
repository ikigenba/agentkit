...
type Conversation struct {
    ...
    Model    string      // swappable between turns; free-flow — any id, the provider judges it (Decision 5)
    Pricing  *Pricing    // sibling of Model: consumer-supplied rates for cost resolution (Decision 16); nil → resolution falls through
...
conv := &agentkit.Conversation{
    Provider: anthropic.New(anthropic.APIKey(key)), // handle carries credentials (Decision 5)
...
Mid-conversation switching is field mutation between turns (`conv.Provider = google.New(google.APIKey(key)); conv.Model = "gemini-2.5-flash"`), with `History` carried over untouched.
...
**Verification.**
- R-ZWV0-CY54 — `Send` on a `Conversation` missing a required config field (provider or model) surfaces `ErrInvalidConfig` (matchable via `errors.Is`) through the returned `Stream` rather than panicking or issuing a malformed provider call. (Credential validity belongs to the operation that uses the credential, never to the constructor — Decision 38.)
