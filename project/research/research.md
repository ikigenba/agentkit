...
| **claude-opus-5** | effort (`output_config.effort`) + `thinking` on/off | enum | `low` `medium` `high` `xhigh` `max` | fixed `medium` (re-measured 2026-08-16: unset reasons 4/4, all five efforts 200, bogus 400 naming the five, `thinking:{type:"disabled"}` zeroes it) | yes | **yes** |
...
A full audit of the OpenRouter routes that are secondaries of natively-served tracked models: the documented per-model `reasoning` descriptor from `GET /api/v1/models`, plus live confirmation probes (13 requests per route — silence ×2, seven effort values, two budgets, both toggles; ~280 requests across the original 22-route sweep). The rates gathered in the same pass live in §6.5. This subsection ends in the **resolved offering table** the catalog transcribes; the resolution rule it applies is D26's ("documented and confirmed; ambiguity resolves toward the native provider; never from acceptance alone"). The `anthropic/claude-opus-5` route was added later (descriptor pulled live 2026-08-16): its documented descriptor matches Sonnet 5's exactly (`mandatory:false, default_enabled:true, default_effort:high, supported_efforts` = the five), its **native** behavior was live-confirmed the same day (§7.1), and its routed vocabulary matches native by pass-through — so it resolves as Sonnet 5 does (Fixed(medium), disable-able). The 13-probe OpenRouter-side confirmation sweep for this one route is a release-time re-verify item; nothing in the resolution depends on it.
...
| claude-opus-5 | Enum effort: low,medium,high,xhigh,max | f | t | Fixed(medium) | `default_enabled:true` agrees with native on-ness; value → native (medium); routed offering mirrors native |
