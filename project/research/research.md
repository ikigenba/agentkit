...
- **Models.** Tracked ids = `claude-opus-4-8` (1M ctx), `claude-opus-5` (1M ctx), `claude-sonnet-4-6`, `claude-sonnet-5`, `claude-haiku-4-5`, `claude-fable-5` (§6.5). Opus 4.8 is the safe default top tier; **`claude-opus-5` is the newer Opus flagship** (released 2026-07-24, same `$5/$25` rate as Opus 4.8), tracked alongside it. **Reasoning control (see §7.1 for the full native spec):** Opus 4.8, Opus 5, and Sonnet 4.6 take a native `output_config.effort` enum plus a `thinking` on/off toggle (adaptive-only when on); **Haiku 4.5 has no `effort` field** — its only reasoning-depth control is `thinking:{type:"enabled",budget_tokens}`. All *can* be disabled (omit `thinking` / `type:"disabled"`); none of Opus 4.8 / Opus 5 is always-on (that is Fable 5 / Mythos 5). The two Opus tiers differ in their **unset default**: **Opus 4.8 does not reason when unset (off, live-confirmed 2026-08-16, 4/4); Opus 5 runs adaptive thinking by default at effort `high`** — per Anthropic's published migration guide, *"The effort parameter default on Claude Opus 5 is `high` on the Claude API and Claude Code"* and *"on Claude Opus 5, [requests without a `thinking` field] run with adaptive thinking."* (Sonnet 5 shares the on-by-default posture but documents a `medium` default effort.) One disable caveat, documented and not modeled by the flat `CanDisable` bool: `thinking:{type:"disabled"}` is accepted only at effort `high` or below; combined with `xhigh`/`max` it returns 400. `budget_tokens` is removed on Opus 4.8 (400). Snapshot-id nuance: Opus 4.8 / Sonnet 4.6 are genuinely **dateless pinned snapshots**, and `claude-opus-5` likewise resolves dateless (OpenRouter's dated canonical is `anthropic/claude-opus-5-20260723`); **`claude-haiku-4-5` is an alias for the dated canonical `claude-haiku-4-5-20251001`** (both resolve). `claude-fable-5` is always-on reasoning — it is the model that cannot disable thinking, not Opus 4.8 or Opus 5.
...
| claude-opus-5 | 5000 | 500 | 6250 | 10000 | 25000 |
...
| claude-opus-5 | 1,000,000 | 5000 | 500 | 6250 | 25000 | = native rates (OpenRouter descriptor 2026-08-16) |
...
| **claude-opus-5** | effort (`output_config.effort`) + `thinking` on/off | enum | `low` `medium` `high` `xhigh` `max` | **fixed `high`** (Anthropic docs: adaptive-on default, effort default `high` on the Claude API/Claude Code; live 2026-08-16: all five efforts 200, bogus 400 naming the five, unset thinks) | yes | **yes** (only at effort ≤ `high`; `disabled`+`xhigh`/`max` → 400, per docs) |
...
A full audit of the OpenRouter routes that are secondaries of natively-served tracked models: the documented per-model `reasoning` descriptor from `GET /api/v1/models`, plus live confirmation probes (13 requests per route — silence ×2, seven effort values, two budgets, both toggles; ~280 requests across the original 22-route sweep). The rates gathered in the same pass live in §6.5. This subsection ends in the **resolved offering table** the catalog transcribes; the resolution rule it applies is D26's ("documented and confirmed; ambiguity resolves toward the native provider; never from acceptance alone"). The `anthropic/claude-opus-5` route was added later (descriptor pulled live 2026-08-16): its documented descriptor is `mandatory:false, default_enabled:true, default_effort:high, supported_efforts` = the five, its **native** behavior was live-confirmed the same day (§7.1), and its routed vocabulary matches native by pass-through. The default effort is settled by Anthropic's **published** docs, not by probe counts (which are non-monotonic and unreliable here): native default effort `high`, adaptive-on — and OpenRouter's `default_effort:high` agrees — so it resolves to **Fixed(high)**, disable-able. The 13-probe OpenRouter-side confirmation sweep for this one route is a release-time re-verify item; nothing in the resolution depends on it.
...
| anthropic/claude-opus-5 | max,xhigh,high,medium,low | high | true | false |
...
| claude-opus-5 | Enum effort: low,medium,high,xhigh,max | f | t | Fixed(high) | Anthropic docs: native default effort `high`, adaptive-on; OpenRouter `default_enabled:true`/`default_effort:high` agree; routed offering mirrors native |
