...
`NewTool[In]` reflects `In` into a schema that contains only subset constructs: nested structs inlined (no `$defs`, no `$ref`), no `$schema`, and no `additionalProperties` — that key is adapter-owned (D22) and must never be authored. A field is required exactly when its `json` tag lacks `,omitempty` and its type is not a pointer. Property metadata comes from `jsonschema` struct tags limited to the subset vocabulary (`description`, `title`, `enum`, `const`, `format`, `pattern`, `minLength`, `maxLength`, `minItems`, `default`); `required` is no longer a tag key. The derived schema stays cached and byte-stable, and returned bytes stay defensively copied.
...
- `R-Y0GH-4VEM` — subset tag keys appear verbatim in the derived schema; an out-of-subset key such as `minimum` leaves the tool rejected at the `Send` gate naming that key (assert the reflector emits it verbatim so Phase 90's gate can reject it).
