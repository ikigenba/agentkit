...
*Realizes a slice of design Decision 22 (the google renderer: R-XVKV-LSFU, R-2XB1-IV26) and a slice of Decision 13 (google's live tool-call cell: R-Y5C2-NYDE). Depends on Phase 90.*
...
- `R-2XB1-IV26` — a construct-complete canonical schema arrives with every subset construct in proto form: `oneOf` as `anyOf`, `const` as one-element `enum`, nullable union as `type` + `nullable: true`, `$ref` inlined, and `pattern`, `minLength`, `maxLength`, `minItems`, `title`, `default`, `format`, `enum`, `description` all present (the keys the superseded converter dropped).
