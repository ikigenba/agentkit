...
Cut the patch release that bundles the two maintenance fixes from D21 and D22. The version is carried by git tags (`v0.1.0`/`v0.1.1`/`v0.1.2` exist; `v0.1.2` is current); there is no version constant in code to edit, and `project/product/README.md`'s "starting version `v0.1.0`" is a historical contractual constant that stays untouched.
