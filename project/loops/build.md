...
- **Language / layout:** Go 1.26; module `github.com/ikigenba/agentkit`. Package `agentkit` lives at the module **root** and is built across several phases. Leaf provider sub-packages: `anthropic/`, `openai/`, `zai/`, `google/`, `openrouter/`, `openai/subscription/`; the advisory model catalog under `catalog/`. Non-consumer-importable shared internals under `internal/`: `internal/httpx`, `internal/sse`, `internal/openaicompat`, `internal/mcp`, `internal/retry`. Public symbols are named so their purpose is clear with no package-name stutter (`agentkit.Conversation`, not `agentkit.AgentKitState`).
